...
export interface StreamTranscriptSegment {
  ...
  audioOffset: number;
  is_final: boolean;
  metadata?: { emotion?: string; intent?: string; age?: string; gender?: string };
  metadata_probs?: {
...
export interface LiveAssistDone {
  ...
  suggestions: string[];
  knowledgeItems: Array<{ id: string; title: string; content: string; detail?: string; source?: string }>;
  actionItems: Array<{ id: string; title: string; type?: string; description?: string; priority?: number }>;
  keywords: string[];
...
export interface LiveAssistCallbacks {
  ...
  onMemory?: (data: { items: LiveAssistMemoryItem[] }) => void;
  onAction?: (data: { items: LiveAssistDone["actionItems"] }) => void;
