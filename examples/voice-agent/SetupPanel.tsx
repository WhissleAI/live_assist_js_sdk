...
import { isSupported, extractText, generateId } from "./lib/documents";
...
export default function SetupPanel({ initialConfig, initialDocuments, onStart }: Props) {
  const [config, setConfig] = useState<VoiceAgentConfig>(() => {
    if (!initialConfig.scenarioId || !initialConfig.systemPrompt) {
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    for (const file of arr) {
      if (!isSupported(file)) {
        setError(`Unsupported file type: ${file.name}. Use text-based files or PDFs.`);
        continue;
      }
      const maxSize = file.name.toLowerCase().endsWith(".pdf") ? 10 * 1024 * 1024 : 2 * 1024 * 1024;
      if (file.size > maxSize) {
        setError(`File too large: ${file.name}. Max ${file.name.toLowerCase().endsWith(".pdf") ? "10MB" : "2MB"}.`);
        continue;
      }
      try {
    ...
    <div className="setup-root">
      <div className="setup-container">
        <div className="setup-header">
          ...
          <h1>Voice Agent</h1>
          <p className="setup-subtitle">Talk to AI with your voice. Choose a scenario or configure your own.</p>
        </div>
          ...
          <div className="setup-presets">
            {PRESETS.map((p) => (
              ...
              >
                ...
                <div className="setup-preset-text">
                  <div className="setup-preset-name">{p.name}</div>
                  <div className="setup-preset-desc">{p.description}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        {/* No client-side API keys needed — LLM and TTS keys are configured server-side */}
        ...
        <div className="setup-section">
          ...
          <textarea className="setup-textarea" value={config.systemPrompt} onChange={(e) => update("systemPrompt", e.target.value)} rows={4} placeholder="Instructions for the AI assistant..." />
        </div>
        ...
        <div className="setup-section">
          <label className="setup-label">
            ...
            <span className="setup-label-hint">(optional)</span>
          </label>
          <div
          ...
          >
            ...
            <span className="setup-dropzone-hint">.pdf, .txt, .md, .csv, .json and more</span>
          </div>
          <input ref={fileInputRef} type="file" multiple accept=".pdf,.txt,.md,.csv,.json,.html,.xml,.yaml,.yml,.log,.js,.ts,.py" style={{ display: "none" }} onChange={(e) => e.target.files && handleFiles(e.target.files)} />

          {documents.length > 0 && (
            <div className="setup-docs">
              {documents.map((doc) => (
                <div key={doc.id} className="setup-doc">
