...
export function TranscriptView({
  ...
  return (
    <div>
      ...
      <div ref={scrollRef} style={{ maxHeight, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
        {filtered.length === 0 && (
...
export function EmotionTimelineBar({
  ...
  if (filtered.length === 0 || durationSec <= 0) return null;
  ...
  for (let t = 0; t <= durationSec; t += step) rulerTicks.push(t);
  if (rulerTicks[rulerTicks.length - 1] !== durationSec) rulerTicks.push(durationSec);

  const usedEmotions = useMemo(() => {
    const set = new Set<string>();
    for (const e of filtered) {
      const em = e.metadata?.emotion;
      if (em) set.add(em.toUpperCase());
    }
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      {/* Ruler */}
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#9ca3af", marginBottom: 3, fontVariantNumeric: "tabular-nums" }}>
        {rulerTicks.map((t) => (
      ...
      <div style={{ position: "relative" }}>
        <div
          ...
          aria-label="Timeline by emotion"
        ...
        >
          {segments.map((s, i) => {
            ...
            return (
              <div
                key={i}
                title={`${s.entry.metadata?.emotion ?? "—"} | ${s.entry.metadata?.intent ?? "—"} | ${formatTime(s.start)}–${formatTime(s.end)} | ${(s.entry.text || "").slice(0, 40)}${(s.entry.text || "").length > 40 ? "…" : ""}`}
                style={{
                  width: `${widthPct}%`,
                  minWidth: 2,
                  background: s.color,
        ...
        {/* Playhead */}
        {currentTimeSec >= 0 && currentTimeSec <= durationSec && (
          <>
            <div
              aria-hidden
              style={{
      ...
      {usedEmotions.length > 0 && (
        <div style={{ display: "flex", gap: 10, marginTop: 4, flexWrap: "wrap" }}>
          {usedEmotions.map((l) => (
            <span key={l.key} style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 10, color: "#9ca3af" }}>
