...
function makeItem(title: string): AgendaItem {
  return {
...
export function getDefaultAgendaItems(): AgendaItem[] {
...
export function getStoredAgenda(): AgendaItem[] {
  ...
  try {
    ...
    if (!raw) return [];
    const arr = JSON.parse(raw) as AgendaItem[];
...
export function setStoredAgenda(items: AgendaItem[]): void {
  if (typeof window === "undefined") return;
  try {
